...
| W2 | blocked | tester/W2 → coordinator | 公開候補 `.local/installer-beta-final`（固定コミット `c729da4dc8fc0df163cd449eef5c90950cfa0c81`、MSI SHA-256 `A9619B7BFCB72C6E554B3BAF700D8EA30657DEC50296D1E999CD644B06F4DF49`、Setup SHA-256 `B37CBC20CFD2D9E5A7349D7B45CB64E27AB09111EED04F47D28817B653E0854A`、UpgradeCode は旧候補と同一 `{381B4CC9-ABAA-4AB2-9DC8-FCA54CE3B964}`、ProductCode `{FBDCE95B-46CA-4959-8D36-26ABEE793117}`）の install / uninstall / reinstall / rollback 検証。Setup.exe の実際の一操作導入を確認し、**旧 `C:\Program Files (x86)\KanaAI` から新 x64 `C:\Program Files\KanaAI` への移行を必ず実測**する（UpgradeCode 同一で MajorUpgrade は効く設計だが別ディレクトリ移行は未検証）。**2026-09-26 更新（blocked）**: ハーネス側の COM 経路は実測で修正済み（`d98771a`、self test 73/73）。しかし**この machine は install state を回答しない**ため W2 を成立させられない。`MsiQueryProductState` が有効な全 product code に対し `ERROR_ACCESS_DENIED`（P/Invoke 3 形、管理権限有無のいずれでも同一。無効名では -1 を返すため dispatch は正しい）。`Installer\ProductInfo('InstallState')` も全 product で例外。`HKLM\...\Installer\Components` キー自体が不在（一方 `Classes\Installer\Products` 171件・`Features` 169件は存在）。この machine に KanaAI が**2件登録**され `{FBDCE95B-…}` の cached MSI と `InstallDate=20260926` があるのに `C:\Program Files\KanaAI` は不在。**-blocked の解除条件**: (a) 不在原因を特定し修復する、または (b) 回答する machine（クリーン VM を含む）に移す。`-Execute` は回答不能を検出すると `INSTALL-STATE-UNDETERMINED` で **phase に触れる前に exit 2** する（`b0092f9`）。UAC 承認と machine lock 専有が必要 | machine lock（専有）、UAC 承認、実行前のユーザー承認、**上記 (a)/(b) の決定** |
