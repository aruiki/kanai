...
| W2 | ready（machine 障害は撤回） | tester/W2 → coordinator | 公開候補 `.local/installer-beta-final`（固定コミット `c729da4dc8fc0df163cd449eef5c90950cfa0c81`、MSI SHA-256 `A9619B7BFCB72C6E554B3BAF700D8EA30657DEC50296D1E999CD644B06F4DF49`、Setup SHA-256 `B37CBC20CFD2D9E5A7349D7B45CB64E27AB09111EED04F47D28817B653E0854A`、UpgradeCode は旧候補と同一 `{381B4CC9-ABAA-4AB2-9DC8-FCA54CE3B964}`、ProductCode `{FBDCE95B-46CA-4959-8D36-26ABEE793117}`）の install / uninstall / reinstall / rollback 検証。Setup.exe の実際の一操作導入を確認し、**旧 `C:\Program Files (x86)\KanaAI` から新 x64 `C:\Program Files\KanaAI` への移行を必ず実測**する。**2026-09-26 更新**: ハーネス側は修正済み（`d98771a`）。**「この machine は install state を回答しない」という block 理由は撤回した** — 原因は coordinator 自身の P/Invoke 宣言ミスで、`MsiQueryProductStateW` は 1 引数・戻り値がそのまま INSTALLSTATE（out パラメータは存在しない）。宣言を直すと実機は `DEFAULT`（= installed）を返し、`Find-KanaAiLifecycleInstalledProducts` は `scanned=182 matched=2`、entry point の `^(DEFAULT|LOCAL)$` に 2 件一致することを**実測済み**。**残る block は証跡の欠落のみ**。ただしこの machine には KanaAI が**2件登録**されており `{FBDCE95B-…}` は cached MSI と `InstallDate=20260926` があるのに `C:\Program Files\KanaAI` が不在（install 成功 event 1033 はある）。**実行前の判断が要る**: (a) 既存2件を残し `-AllowPreexistingTarget` / `-AllowUnexpectedExistingInstall` で「導入済みからの upgrade」を観測する、(b) 両方を一度アンインストールして clean install から始める。UAC 承認と machine lock 専有が必要。**自律 agent は停止済みだが、mutation 前にユーザーの在席を確認すること** | machine lock（専有）、UAC 承認、実行前のユーザー承認、上記 (a)/(b) の決定 |
...
| A2-08 | blocked | — | **R-AI監査が確定した実装机のAI経路欠陥。** (1) **CRITICAL**: 実装机のAI経路は**構造的に起動不能**であり、再ビルドでは直らない。coordinator が実アーティファクトで確定した。**需要側**: `installed_ai.rs:444-448` は `<exe>\ai\manifest-v1.json` と `<exe>\ai\STAGING-RECEIPT.json` を `read_config` して `?` で失敗させる。**供給側**: `build-windows-installer.ps1:1367` が `throw 'The raw local AI staging receipt must never become an MSI payload file.'`、`:1368` が `throw 'The sanitized local AI package manifest must never become an MSI payload file.'` で payload を禁じ、`:1714-1716` は sanitized manifest を staging にコピーするだけで MSI にはしない。**実測**: 実在する AI 同梱 MSI（`.local/installer-ai-beta/KanaAI-0.1.0-x64.msi`、1,124,446,208 bytes）の File table は 68 行 = AI 56 行（`kanai-broker.exe`、Qwen2.5-1.5B GGUF、llama.cpp runtime 51 ファイル、license/notice 3）+ Mozc 12 行。**`manifest-v1.json` / `STAGING-RECEIPT.json` / `PACKAGE-MANIFEST.json` は 1 件も含まれていない**。`manifest-v1.json` は `.local` 全体に 0 件で、staging された成品にも存在しない。**決定が要る点**: broker が読む設定の供給源 adjudicate する。候補は (a) 起動 plan をビルド時にバイナリへ embed、(b) manifest/receipt と**別**の、secret を含まない最小設定を payload として送る（現行方針を明示的に改訂）、(c) 実行時にローカルの staging ツリーを参照（配布物では不可）。**セキュリティモデルの再設計を伴うので coordinator/ユーザーと確定する。fallback で糊塗しない。** また payload と broker の要求するファイル名が一致することを検証する**回帰テストが未整備**であることもfinding。 (2) HIGH `verify_connection` のdead code と (3) HIGH Windows既定 `LocalQualityOnly` は **A2-08a で完了**。**残**: (4) **F1の残余競窓** — `verify_endpoint()` の検証ソケットと reqwest の送信ソケットが別接続で、その間のTOCTOUは `local_model.rs`（所有外）に接続層のseamが要るまで排除できない。現状は (1) でAI経路が起動しないため到達不能だが、**AI有効化の前に必ず塞ぐ**。(5) H-2 `%TEMP%` が日本語アカウントで非ASCII→`KeyFilePathRejected` で恒久off、(6) H-3 既定deadline 250ms に対し実測 1.46s | A2-06, A2-08a |
...
| A2-06 | done | coordinator が実コードで再確認 | composition root は**既に統合済み**で、WORK_QUEUE の `ready` は古かった。`kanai-broker.rs:246-259` が `installed_ai::policy(KANAI_BROKER_ENHANCEMENT)` を解決し、`SwitchableBackend::default()` を作り、`BackgroundAi::start(backend, policy)` で AI を spawn し、終了時に `ai.shutdown().await` する。`installed_ai.rs` 側に `run()` / `watch_installed_runtime()` / `start_pinned_ai_runtime()` / `reserve_loopback_port()` / `KEY_SWEEP_BUDGET` による stale key directory 回収 / `KeyDirectory` による CREATE_NEW・owner-only key file が実装済みで、secret は key file に書いて権限制限し process 引数・ログ・pipe 応答へ出さない。**2026-09-26 coordinator が Rust 全 gate を再実測**: `cargo fmt --all -- --check` exit 0、`cargo check --workspace --all-targets --locked` exit 0、`cargo clippy --workspace --all-targets --locked -- -D warnings` exit 0、`cargo test --workspace --locked` **181 passed / 0 failed / 2 ignored** exit 0（`installed_ai::tests::available_before_startup_and_failed_model_does_not_block_slot` を含む）。**残**: 起動が実装机で成立するかどうかは A2-08 の payload 欠落が支配的なので未解決。integration は済んでいるが**成功経路は A2-08 を直すまで一度も実行されない** | A2-04, A2-05 |
