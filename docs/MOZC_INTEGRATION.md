...
flowchart LR
    ...
    M -->|bounded multi-session bridge| S[mozc_server]
...
- The current Rust `MozcSessionBackend` uses one bounded bridge process with
  explicit `open`, `key`, `edit`, `convert`, `commit`, `cancel`, and `close`
  commands. The C++ `SessionHandler` protocol keeps the old one-shot facade for
  compatibility, caps total upstream sessions at 64 (including the legacy
  compatibility session), and runs every session in incognito/no-history mode.
  Rust still owns the broker session ID and
  monotonic generation; a commit is accepted only with the matching active
  conversion snapshot. A rendered non-ASCII composition is fed through Mozc
  `AS_IS` key events before the bounded space conversion; this avoids a full
  `UPDATE_COMPOSITION` re-segmentation on every request. The checked Linux
  bridge receipt records the before/after observation, but it is not a Windows
  performance acceptance result.

The internal bridge line protocol is tab-separated and percent-encodes every
UTF-8 field. The explicit form is:

```text
open   <bridge-session>
key    <bridge-session> <expected-generation> <kind> <value>
edit   <bridge-session> <expected-generation> <insert|replace|delete|reset> ...
convert <bridge-session> <romaji-or-composition> <before> <after> <generation>
commit <bridge-session> <mozc-candidate-id> <generation>
cancel <bridge-session> [generation]
close  <bridge-session> [generation]
```

Unknown, duplicate, over-limit, malformed, and stale requests fail without
falling back to another session. The legacy `ping/reset/convert/commit/shutdown`
forms continue to target the compatibility session (`0`). The bridge response
is JSON and remains bounded to 1 MiB; the Rust adapter checks generation and
candidate identity again before exposing a result.
...
- The repository-owned bridge receipt is generated by
  `scripts/benchmark-mozc-bridge.py`; it measures real child-process conversion
  only and must not be presented as AI ranking or Windows TSF latency.
